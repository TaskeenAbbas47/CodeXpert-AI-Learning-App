...
import {
  ...
  updateOverallCourseProgress, // ✅ 1. Import this
...
export default function JavascriptCourse({ _navigation }: any) {
  ...
  // --- 1. Load JS Course Data ---
  useEffect(() => {
    let mounted = true;
    const init = async () => {
      try {
        const data = await loadCourse("javascript_linear_mastery");
        ...
        if (mounted) {
            if (data) {
                ...
                console.error("Javascript Course data missing.");
            }
        }
      } catch (e) {
  ...
  useEffect(() => {
    if (user && course) {
        updateOverallCourseProgress(user.uid, "javascript", overallProgress);
    }
  ...
  const handleLessonComplete = async () => {
    ...
    try {
      await markLessonComplete(uid, "javascript", sectionId, lessonId);
      ...
      if (next) {
        await unlockNextLesson(uid, "javascript", sectionId, String(next.id));
    ...
    <View style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>
          {selectedLesson ? selectedLesson.title : "JavaScript Brain"}
        </Text>
      </View>
                  ...
                  <View style={styles.progressBarContainer}>
                    <View
                      style={[
                        ...
                        { width: `${progress}%`, backgroundColor: "#F7DF1E" }, // ✅ JS Yellow
                      ]}
                    />
                  </View>
      ...
      {selectedSection && !selectedLesson && (
        <CourseRoadmap
          ...
          iconLanguage={require("../Assets/js.png")}
