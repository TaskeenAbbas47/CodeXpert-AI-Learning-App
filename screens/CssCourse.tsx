...
export default function CssCourse({ _navigation }: any) {
  ...
  // --- 1. Load CSS Course Data ---
  useEffect(() => {
    let mounted = true;
    const init = async () => {
      try {
        const data = await loadCourse("css_linear_mastery");
        ...
        if (mounted) {
            if (data) {
                ...
                console.error("CSS Course data missing.");
            }
        }
      } catch (e) {
  ...
  useEffect(() => {
    if (user && course) {
        updateOverallCourseProgress(user.uid, "css", overallProgress);
    }
  ...
  const handleLessonComplete = async () => {
    ...
    try {
      await markLessonComplete(uid, "css", sectionId, lessonId);
      ...
      if (next) {
        await unlockNextLesson(uid, "css", sectionId, String(next.id));
    ...
    <View style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>
          {selectedLesson ? selectedLesson.title : "CSS3 Stylist"}
        </Text>
      </View>
                  ...
                  <View style={styles.progressBarContainer}>
                    <View
                      style={[
                        ...
                        { width: `${progress}%`, backgroundColor: "#2965f1" }, // ✅ CSS Blue
                      ]}
                    />
                  </View>
      ...
      {selectedSection && !selectedLesson && (
        <CourseRoadmap
          ...
          iconLanguage={require("../Assets/css.png")}
